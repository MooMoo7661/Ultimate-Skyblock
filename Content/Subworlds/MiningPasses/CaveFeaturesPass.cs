...
{
    public class CaveFeaturesPass : GenPass
    {
        public CaveFeaturesPass(string name, double loadWeight) : base(name, loadWeight) { }
        ...
        protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
        {
            for (int x = 50; x < Main.maxTilesX - 50; x++)
            {
                for (int y = 50; y < Main.maxTilesY - 50; y++)
                {
                    ...
                    if (y < Main.UnderworldLayer && WorldGen.genRand.NextBool(50) && GenUtils.SuitableFor2x2(x, y))
                    {
