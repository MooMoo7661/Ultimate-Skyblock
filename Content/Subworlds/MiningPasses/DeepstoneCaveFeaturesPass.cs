    ...
    public class DeepstoneCaveFeaturesPass : GenPass
    {
        ...
        protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            for (int x = 0; x < Main.maxTilesX; x++)
            {
                ...
                {
                    if (WorldGen.genRand.NextBool(14) && !Framing.GetTileSafely(x, y - 1).HasTile && Framing.GetTileSafely(x - 1, y).HasTile && Framing.GetTileSafely(x + 1, y).HasTile && Framing.GetTileSafely(x, y).TileType == ModContent.TileType<DeepstoneTile>())
                    {
            ...
            for (int x = 0; x < Main.maxTilesX; x++)
            {
                for (int y = 0; y < Main.maxTilesY; y++)
                {
                    if (WorldGen.genRand.NextBool(17) && !Framing.GetTileSafely(x, y + 1).HasTile && Framing.GetTileSafely(x, y).TileType == ModContent.TileType<DeepstoneTile>())
                    {
                        int height = WorldGen.genRand.Next(8, 15);
                        for (int i = 1; i < height; i++)
                        {
